...
class RenderInterface_DX12WL : public ExtendedRenderInterface {
private:
...
public:
	...
	public:
		...
		void Initialize(D3D12MA::Allocator* p_allocator, OffsetAllocator::Allocator* p_offset_allocator_for_descriptor_heap_srv_cbv_uav,
			...
			D3D12_CPU_DESCRIPTOR_HANDLE* p_handle, RenderInterface_DX12WL* p_renderer,
	...
	private:
		...
		RenderInterface_DX12WL* m_p_renderer;
	...
	*/
	...
	public:
		...
		void Initialize(RenderInterface_DX12WL* p_owner);
		void Shutdown();
...
public:
	RenderInterface_DX12WL(void* p_window_handle, const Backend::RmlRendererSettings& settings);
	~RenderInterface_DX12WL();
	...
	// Draws the result to the backbuffer.
	void EndFrame() override;

	// Optional, can be used to clear the active framebuffer.
	void Clear() override;

	// Re-map existing VB with new vertex data (stub — not yet implemented).
