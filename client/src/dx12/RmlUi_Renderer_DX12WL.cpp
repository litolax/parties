#include "RmlUi_Renderer_DX12WL.h"
...
public:
	...
	void Set_Texture(RenderInterface_DX12WL::TextureHandleType* p_texture) { m_p_texture = p_texture; }
	RenderInterface_DX12WL::TextureHandleType* Get_Texture() const { return m_p_texture; }
...
private:
	...
	int m_height;
	int m_id;
	RenderInterface_DX12WL::TextureHandleType* m_p_texture;
...
RenderInterface_DX12WL::RenderInterface_DX12WL(void* p_window_handle, const Backend::RmlRendererSettings& settings) :
...
{
	static_assert(RenderInterface_DX12WL::NumPrograms == static_cast<size_t>(ProgramId::Count),
	...
	m_precompiled_fullscreen_quad_geometry = RenderInterface_DX12WL::CompileGeometry(mesh.vertices, mesh.indices);
...
RenderInterface_DX12WL::~RenderInterface_DX12WL()
{
...
RenderInterface_DX12WL::operator bool() const
{
	RMLUI_ZoneScopedN("DirectX 12 - operator bool()");
	return !!(m_p_device);
}

void RenderInterface_DX12WL::BeginFrame()
{
...
void RenderInterface_DX12WL::EndFrame()
{
...
void RenderInterface_DX12WL::Clear()
{
...
void RenderInterface_DX12WL::SetViewport(int viewport_width, int viewport_height, bool force)
{
	RMLUI_ZoneScopedN("DirectX 12 - SetViewport");
	if (viewport_width <= 0 || viewport_height <= 0)
		return;

	if (force || m_width != viewport_width || m_height != viewport_height)
	{
		Flush();
...
void RenderInterface_DX12WL::RenderGeometry(Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation, Rml::TextureHandle texture)
{
...
Rml::CompiledGeometryHandle RenderInterface_DX12WL::CompileGeometry(Rml::Span<const Rml::Vertex> vertices, Rml::Span<const int> indices)
{
...
void RenderInterface_DX12WL::ReleaseGeometry(Rml::CompiledGeometryHandle geometry)
{
...
void RenderInterface_DX12WL::UpdateGeometryVertices(
...
void RenderInterface_DX12WL::SetScissorRegion(Rml::Rectanglei region)
{
...
void RenderInterface_DX12WL::EnableClipMask(bool enable)
{
...
void RenderInterface_DX12WL::RenderToClipMask(Rml::ClipMaskOperation mask_operation, Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation)
{
...
Rml::TextureHandle RenderInterface_DX12WL::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source)
{
...
Rml::TextureHandle RenderInterface_DX12WL::GenerateTexture(Rml::Span<const Rml::byte> source_data, Rml::Vector2i source_dimensions)
{
...
void RenderInterface_DX12WL::ReleaseTexture(Rml::TextureHandle texture_handle)
{
...
void RenderInterface_DX12WL::SetTransform(const Rml::Matrix4f* transform)
{
...
RenderInterface_DX12WL::RenderLayerStack::RenderLayerStack() :
...
void RenderInterface_DX12WL::RenderLayerStack::Initialize(RenderInterface_DX12WL* p_owner)
{
	RMLUI_ZoneScopedN("DirectX 12 - RenderLayerStack::Initialize");
	RMLUI_ASSERTMSG(p_owner, "you must pass a valid pointer of RenderInterface_DX12WL instance");
	if (p_owner)
	{
...
void RenderInterface_DX12WL::RenderLayerStack::Shutdown()
{
	RMLUI_ZoneScopedN("DirectX 12 - RenderLayerStack::Shutdown");
	DestroyFramebuffers();
}

Rml::LayerHandle RenderInterface_DX12WL::RenderLayerStack::PushLayer()
{
...
void RenderInterface_DX12WL::RenderLayerStack::PopLayer()
{
...
const Gfx::FramebufferData& RenderInterface_DX12WL::RenderLayerStack::GetLayer(Rml::LayerHandle layer) const
{
...
const Gfx::FramebufferData& RenderInterface_DX12WL::RenderLayerStack::GetTopLayer() const
{
...
const Gfx::FramebufferData& RenderInterface_DX12WL::RenderLayerStack::Get_SharedDepthStencil_Layers()
{
...
Rml::LayerHandle RenderInterface_DX12WL::RenderLayerStack::GetTopLayerHandle() const
{
...
void RenderInterface_DX12WL::RenderLayerStack::SwapPostprocessPrimarySecondary()
{
...
void RenderInterface_DX12WL::RenderLayerStack::BeginFrame(int width_new, int height_new)
{
...
void RenderInterface_DX12WL::RenderLayerStack::EndFrame()
{
...
void RenderInterface_DX12WL::RenderLayerStack::DestroyFramebuffers()
{
...
const Gfx::FramebufferData& RenderInterface_DX12WL::RenderLayerStack::EnsureFramebufferPostprocess(int index)
{
...
void RenderInterface_DX12WL::RenderLayerStack::CreateFramebuffer(Gfx::FramebufferData* p_result, int width, int height, int sample_count,
...
void RenderInterface_DX12WL::RenderLayerStack::DestroyFramebuffer(Gfx::FramebufferData* p_data)
{
...
Rml::LayerHandle RenderInterface_DX12WL::PushLayer()
{
...
void RenderInterface_DX12WL::BlitLayerToPostprocessPrimary(Rml::LayerHandle layer_id)
{
...
void RenderInterface_DX12WL::DrawFullscreenQuad(ConstantBufferType* p_override_constant_buffer)
{
...
void RenderInterface_DX12WL::DrawFullscreenQuad(Rml::Vector2f uv_offset, Rml::Vector2f uv_scaling, ConstantBufferType* p_override_constant_buffer)
{
...
void RenderInterface_DX12WL::BindTexture(TextureHandleType* p_texture, UINT root_parameter_index)
{
...
void RenderInterface_DX12WL::BindRenderTarget(const Gfx::FramebufferData& framebuffer, bool depth_included)
{
	RMLUI_ASSERTMSG(m_p_command_graphics_list, "early calling must be initialized before calling this method!");
...
void RenderInterface_DX12WL::OverrideConstantBufferOfGeometry(Rml::CompiledGeometryHandle geometry, ConstantBufferType* p_override_constant_buffer)
{
	if (p_override_constant_buffer)
	{
		GeometryHandleType* p_geometry = reinterpret_cast<GeometryHandleType*>(geometry);
...
unsigned char RenderInterface_DX12WL::GetMSAASupportedSampleCount(unsigned char max_samples)
{
...
void RenderInterface_DX12WL::BlitFramebuffer(const Gfx::FramebufferData& source, const Gfx::FramebufferData& dest, int srcX0, int srcY0, int srcX1,
...
void RenderInterface_DX12WL::ValidateTextureAllocationNotAsPlaced(const Gfx::FramebufferData& source)
{
#ifdef RMLUI_DX_DEBUG
...
ID3D12Resource* RenderInterface_DX12WL::GetResourceFromFramebufferData(const Gfx::FramebufferData& data)
{
...
void RenderInterface_DX12WL::RenderBlur(float sigma, const Gfx::FramebufferData& source_destination, const Gfx::FramebufferData& temp,
	const Rml::Rectanglei window_flipped)
{
	...
	for (int i = 0; i < pass_level; i++)
	{
		...
		RenderInterface_DX12WL::TextureHandleType* p_texture = nullptr;
	...
	if (transfer_to_temp_buffer)
	{
		...
		RenderInterface_DX12WL::TextureHandleType* p_texture = source_destination.Get_Texture();
		RMLUI_ASSERTMSG(p_texture, "must be valid!");
...
void RenderInterface_DX12WL::RenderFilters(Rml::Span<const Rml::CompiledFilterHandle> filter_handles)
{
...
void RenderInterface_DX12WL::CompositeLayers(Rml::LayerHandle source, Rml::LayerHandle destination, Rml::BlendMode blend_mode,
...
void RenderInterface_DX12WL::PopLayer()
{
...
Rml::TextureHandle RenderInterface_DX12WL::SaveLayerAsTexture()
{
...
Rml::CompiledFilterHandle RenderInterface_DX12WL::SaveLayerAsMaskImage()
{
...
Rml::CompiledFilterHandle RenderInterface_DX12WL::CompileFilter(const Rml::String& name, const Rml::Dictionary& parameters)
{
...
void RenderInterface_DX12WL::ReleaseFilter(Rml::CompiledFilterHandle filter)
{
	RMLUI_ZoneScopedN("DirectX 12 - ReleaseFilter");
	delete reinterpret_cast<CompiledFilter*>(filter);
}

Rml::CompiledShaderHandle RenderInterface_DX12WL::CompileShader(const Rml::String& name, const Rml::Dictionary& parameters)
{
...
void RenderInterface_DX12WL::RenderShader(Rml::CompiledShaderHandle shader_handle, Rml::CompiledGeometryHandle geometry_handle,
...
void RenderInterface_DX12WL::ReleaseShader(Rml::CompiledShaderHandle effect_handle)
{
	RMLUI_ZoneScopedN("DirectX 12 - ReleaseShader");
	delete reinterpret_cast<CompiledShader*>(effect_handle);
}

bool RenderInterface_DX12WL::IsSwapchainValid() noexcept
{
	RMLUI_ZoneScopedN("DirectX 12 - IsSwapchainValid");
	return (m_p_swapchain != nullptr);
}

void RenderInterface_DX12WL::RecreateSwapchain() noexcept
{
	RMLUI_ZoneScopedN("DirectX 12 - RecreateSwapchain");
	SetViewport(m_width, m_height);
}

ID3D12Fence* RenderInterface_DX12WL::Get_Fence()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_Fence");
	return m_p_backbuffer_fence;
}

HANDLE RenderInterface_DX12WL::Get_FenceEvent()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_FenceEvent");
	return m_p_fence_event;
}

Rml::Array<uint64_t, RMLUI_RENDER_BACKEND_FIELD_SWAPCHAIN_BACKBUFFER_COUNT>& RenderInterface_DX12WL::Get_FenceValues()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_FenceValues");
	return m_backbuffers_fence_values;
}

uint32_t RenderInterface_DX12WL::Get_CurrentFrameIndex()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_CurrentFrameIndex");
	return m_current_back_buffer_index;
}

ID3D12Device* RenderInterface_DX12WL::Get_Device() const
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_Device");
	return m_p_device;
}

RenderInterface_DX12WL::TextureMemoryManager& RenderInterface_DX12WL::Get_TextureManager()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_TextureManager");
	return m_manager_texture;
}

RenderInterface_DX12WL::BufferMemoryManager& RenderInterface_DX12WL::Get_BufferManager()
{
	RMLUI_ZoneScopedN("DirectX 12 - Get_BufferManager");
	return m_manager_buffer;
}

unsigned char RenderInterface_DX12WL::Get_MSAASampleCount() const
{
	return m_msaa_sample_count;
}

void RenderInterface_DX12WL::Set_UserFramebufferIndex(unsigned char framebuffer_index)
{
	m_current_back_buffer_index = static_cast<UINT>(framebuffer_index);
}

void RenderInterface_DX12WL::Set_UserRenderTarget(void* rtv_where_we_render_to)
{
...
void RenderInterface_DX12WL::Set_UserDepthStencil(void* dsv_where_we_render_to)
{
...
bool RenderInterface_DX12WL::CaptureScreen(int& out_width, int& out_height, int& out_num_components, Rml::UniquePtr<Rml::byte[]>& out_data)
{
	out_width = -1;
	out_height = -1;
...
void RenderInterface_DX12WL::Initialize_Device() noexcept
{
...
void RenderInterface_DX12WL::Initialize_Adapter() noexcept
{
...
void RenderInterface_DX12WL::Initialize_DebugLayer() noexcept
{
...
ID3D12CommandQueue* RenderInterface_DX12WL::Create_CommandQueue(D3D12_COMMAND_LIST_TYPE type) noexcept
{
...
void RenderInterface_DX12WL::Initialize_Swapchain(int width, int height) noexcept
{
...
void RenderInterface_DX12WL::Initialize_SyncPrimitives() noexcept
{
...
void RenderInterface_DX12WL::Initialize_SyncPrimitives_Screenshot() noexcept
{
...
void RenderInterface_DX12WL::Initialize_CommandAllocators()
{
...
void RenderInterface_DX12WL::Initialize_Allocator() noexcept
{
...
void RenderInterface_DX12WL::Destroy_Swapchain() noexcept
{
...
void RenderInterface_DX12WL::Destroy_SyncPrimitives() noexcept
{
...
void RenderInterface_DX12WL::Destroy_CommandAllocators() noexcept
{
...
void RenderInterface_DX12WL::Destroy_CommandList() noexcept {}

void RenderInterface_DX12WL::Destroy_Allocator() noexcept
{
...
void RenderInterface_DX12WL::Destroy_SyncPrimitives_Screenshot() noexcept
{
...
void RenderInterface_DX12WL::Flush() noexcept
{
...
uint64_t RenderInterface_DX12WL::Signal(uint32_t frame_index) noexcept
{
...
void RenderInterface_DX12WL::WaitForFenceValue(uint32_t frame_index)
{
...
void RenderInterface_DX12WL::Create_Resources_DependentOnSize() noexcept
{
...
void RenderInterface_DX12WL::Destroy_Resources_DependentOnSize() noexcept
{
...
void RenderInterface_DX12WL::Create_Resource_DepthStencil()
{
...
void RenderInterface_DX12WL::Destroy_Resource_DepthStencil()
{
...
ID3D12DescriptorHeap* RenderInterface_DX12WL::Create_Resource_DescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_DESCRIPTOR_HEAP_FLAGS flags,
...
ID3D12CommandAllocator* RenderInterface_DX12WL::Create_CommandAllocator(D3D12_COMMAND_LIST_TYPE type)
{
...
ID3D12GraphicsCommandList* RenderInterface_DX12WL::Create_CommandList(ID3D12CommandAllocator* p_allocator, D3D12_COMMAND_LIST_TYPE type) noexcept
{
...
void RenderInterface_DX12WL::Create_Resource_RenderTargetViews()
{
...
void RenderInterface_DX12WL::Destroy_Resource_RenderTagetViews()
{
...
void RenderInterface_DX12WL::Destroy_Resource_For_Shaders()
{
...
void RenderInterface_DX12WL::Free_Geometry(RenderInterface_DX12WL::GeometryHandleType* p_handle)
{
...
void RenderInterface_DX12WL::Free_Texture(RenderInterface_DX12WL::TextureHandleType* p_handle)
{
...
void RenderInterface_DX12WL::Update_PendingForDeletion_Geometry()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipelines()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Texture()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Gradient()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Creation()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Passthrough()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Passthrough_NoBlend()
{
	RMLUI_ZoneScopedN("DirectX 12 - Create_Resource_Pipeline_Passthrough");
	RMLUI_ASSERTMSG(m_p_device, "must be valid when we call this method!");
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_ColorMatrix()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_BlendMask()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_Blur()
{
...
void RenderInterface_DX12WL::Create_Resource_Pipeline_DropShadow()
{
...
bool RenderInterface_DX12WL::CheckTearingSupport() noexcept
{
...
IDXGIAdapter* RenderInterface_DX12WL::Get_Adapter(bool is_use_warp) noexcept
{
...
void RenderInterface_DX12WL::PrintAdapterDesc(IDXGIAdapter* p_adapter)
{
...
void RenderInterface_DX12WL::SetScissor(Rml::Rectanglei region, bool vertically_flip)
{
...
void RenderInterface_DX12WL::SubmitTransformUniform(ConstantBufferType& constant_buffer, const Rml::Vector2f& translation)
{
...
void RenderInterface_DX12WL::UseProgram(ProgramId pipeline_id)
{
...
RenderInterface_DX12WL::ConstantBufferType* RenderInterface_DX12WL::Get_ConstantBuffer(uint32_t current_back_buffer_index)
{
...
RenderInterface_DX12WL::BufferMemoryManager::BufferMemoryManager() :
...
bool RenderInterface_DX12WL::BufferMemoryManager::Is_Initialized() const
{
	RMLUI_ZoneScopedN("DirectX 12 - BufferManagerManager::Is_Initialized");
	return static_cast<bool>(m_p_device != nullptr);
}

void RenderInterface_DX12WL::BufferMemoryManager::Initialize(ID3D12Device* p_device, D3D12MA::Allocator* p_allocator,
...
void RenderInterface_DX12WL::BufferMemoryManager::Shutdown()
{
...
void RenderInterface_DX12WL::BufferMemoryManager::Alloc_Vertex(const void* p_data, int num_vertices, size_t size_of_one_element_in_p_data,
...
void RenderInterface_DX12WL::BufferMemoryManager::Alloc_Index(const void* p_data, int num_vertices, size_t size_of_one_element_in_p_data,
	GeometryHandleType* p_handle)
{
...
RenderInterface_DX12WL::GraphicsAllocationInfo RenderInterface_DX12WL::BufferMemoryManager::Alloc_ConstantBuffer(ConstantBufferType* p_resource,
	size_t size)
{
...
void RenderInterface_DX12WL::BufferMemoryManager::Free_ConstantBuffer(ConstantBufferType* p_constantbuffer)
{
...
void RenderInterface_DX12WL::BufferMemoryManager::Free_Geometry(GeometryHandleType* p_handle)
{
...
void* RenderInterface_DX12WL::BufferMemoryManager::Get_WritableMemoryFromBufferByOffset(const GraphicsAllocationInfo& info)
{
...
D3D12MA::Allocation* RenderInterface_DX12WL::BufferMemoryManager::Get_BufferByIndex(int buffer_index)
{
...
void RenderInterface_DX12WL::BufferMemoryManager::Alloc_Buffer(size_t size
#ifdef RMLUI_DX_DEBUG
	,
...
D3D12MA::VirtualBlock* RenderInterface_DX12WL::BufferMemoryManager::Get_AvailableBlock(size_t size_for_allocation, int* result_index)
{
...
D3D12MA::VirtualBlock* RenderInterface_DX12WL::BufferMemoryManager::Get_NotOutOfMemoryAndAvailableBlock(size_t size_for_allocation, int* result_index)
{
...
int RenderInterface_DX12WL::BufferMemoryManager::Alloc(GraphicsAllocationInfo& info, size_t size, size_t alignment)
{
...
void RenderInterface_DX12WL::BufferMemoryManager::TryToFreeAvailableBlock()
{
...
RenderInterface_DX12WL::TextureMemoryManager::TextureMemoryManager() :
...
bool RenderInterface_DX12WL::TextureMemoryManager::Is_Initialized() const
{
	RMLUI_ZoneScopedN("DirectX 12 - TextureMemoryManager::Is_Initialized");
	return static_cast<bool>(m_p_device != nullptr);
}

void RenderInterface_DX12WL::TextureMemoryManager::Initialize(D3D12MA::Allocator* p_allocator,
	...
	ID3D12CommandQueue* p_copy_queue, D3D12_CPU_DESCRIPTOR_HANDLE* p_handle, RenderInterface_DX12WL* p_renderer, size_t size_for_placed_heap)
{
...
void RenderInterface_DX12WL::TextureMemoryManager::Shutdown()
{
...
ID3D12Resource* RenderInterface_DX12WL::TextureMemoryManager::Alloc_Texture(D3D12_RESOURCE_DESC& desc, TextureHandleType* p_impl,
...
ID3D12Resource* RenderInterface_DX12WL::TextureMemoryManager::Alloc_Texture(D3D12_RESOURCE_DESC& desc, Gfx::FramebufferData* p_impl,
...
void RenderInterface_DX12WL::TextureMemoryManager::Free_Texture(TextureHandleType* p_texture)
{
...
void RenderInterface_DX12WL::TextureMemoryManager::Free_Texture(Gfx::FramebufferData* p_impl)
{
...
void RenderInterface_DX12WL::TextureMemoryManager::Free_Texture(TextureHandleType* p_texture, bool is_rt, D3D12MA::VirtualAllocation& allocation)
{
...
bool RenderInterface_DX12WL::TextureMemoryManager::CanAllocate(size_t total_memory_for_allocation, D3D12MA::VirtualBlock* p_block)
{
...
bool RenderInterface_DX12WL::TextureMemoryManager::CanBePlacedResource(size_t total_memory_for_allocation)
{
...
bool RenderInterface_DX12WL::TextureMemoryManager::CanBeSmallResource(size_t base_memory)
{
...
D3D12MA::VirtualBlock* RenderInterface_DX12WL::TextureMemoryManager::Get_AvailableBlock(size_t total_memory_for_allocation, int* result_index)
{
...
void RenderInterface_DX12WL::TextureMemoryManager::Alloc_As_Committed(size_t base_memory, size_t total_memory, D3D12_RESOURCE_DESC& desc,
...
void RenderInterface_DX12WL::TextureMemoryManager::Alloc_As_Placed(size_t base_memory, size_t total_memory, D3D12_RESOURCE_DESC& desc,
	TextureHandleType* p_impl, const Rml::byte* p_data)
{
...
void RenderInterface_DX12WL::TextureMemoryManager::Upload(bool is_committed, TextureHandleType* p_texture_handle, const D3D12_RESOURCE_DESC& desc,
...
size_t RenderInterface_DX12WL::TextureMemoryManager::BytesPerPixel(DXGI_FORMAT format)
{
	RMLUI_ZoneScopedN("DirectX 12 - TextureMemoryManager::BytesPerPixel");
	return BitsPerPixel(format) / static_cast<size_t>(8);
}

size_t RenderInterface_DX12WL::TextureMemoryManager::BitsPerPixel(DXGI_FORMAT format)
{
...
Rml::Pair<ID3D12Heap*, D3D12MA::VirtualBlock*> RenderInterface_DX12WL::TextureMemoryManager::Create_HeapPlaced(size_t size_for_creation)
{
...
D3D12_CPU_DESCRIPTOR_HANDLE RenderInterface_DX12WL::TextureMemoryManager::Alloc_DepthStencilResourceView(ID3D12Resource* p_resource,
...
D3D12_CPU_DESCRIPTOR_HANDLE RenderInterface_DX12WL::TextureMemoryManager::Alloc_RenderTargetResourceView(ID3D12Resource* p_resource,
	D3D12MA::VirtualAllocation* p_alloc)
{
...
// D3D12MemAlloc and offsetAllocator are compiled separately
// (vcpkg D3D12MemoryAllocator library + offsetAllocator.cpp in CMakeLists)
