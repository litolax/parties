...
void RenderInterface_DX12::BeginFrame()
{
	...
	// Wait for the swapchain to signal that a back buffer is available.
	// This paces the CPU to the display refresh rate, preventing frame queuing.
	if (m_frame_latency_waitable)
		WaitForSingleObjectEx(m_frame_latency_waitable, 1000, TRUE);
...
void RenderInterface_DX12::EndFrame()
{
	...
	if (m_p_command_graphics_list)
	{
		...
		D3D12_RESOURCE_BARRIER barrier_transition_from_source_to_rt;
		barrier_transition_from_source_to_rt.Flags = D3D12_RESOURCE_BARRIER_FLAGS::D3D12_RESOURCE_BARRIER_FLAG_NONE;
		barrier_transition_from_source_to_rt.Type = D3D12_RESOURCE_BARRIER_TYPE::D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		barrier_transition_from_source_to_rt.Transition.Subresource = 0;
		barrier_transition_from_source_to_rt.Transition.pResource = p_msaa_texture;
		barrier_transition_from_source_to_rt.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;

		if (m_is_use_msaa)
		{
			barriers[0].Transition.StateAfter = D3D12_RESOURCE_STATE_RESOLVE_SOURCE;
			barriers[1].Transition.StateAfter = D3D12_RESOURCE_STATE_RESOLVE_DEST;
			barrier_transition_from_source_to_rt.Transition.StateBefore = D3D12_RESOURCE_STATE_RESOLVE_SOURCE;
		...
		else
		{
			...
			barrier_transition_from_source_to_rt.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_SOURCE;

			m_p_command_graphics_list->ResourceBarrier(2, barriers);
			m_p_command_graphics_list->CopyResource(p_postprocess_texture, p_msaa_texture);
		}

		m_p_command_graphics_list->ResourceBarrier(1, &barrier_transition_from_source_to_rt);
		...
		offscreen_texture_barrier_for_shader.Transition.StateBefore = m_is_use_msaa ? D3D12_RESOURCE_STATE_RESOLVE_DEST : D3D12_RESOURCE_STATE_COPY_DEST;
		...
		// Reset stencil ref and scissor to fullscreen before the final blit.
		// RmlUi's clip mask operations may have changed the stencil ref to non-zero,
		// and scissor regions may still be active from the last draw call.
		// The Passthrough PSO uses stencil EQUAL test, so ref must match the
		// cleared backbuffer DSV (0). Scissor must cover the entire backbuffer.
...
void RenderInterface_DX12::SetViewport(int viewport_width, int viewport_height)
{
	...
	if (m_width != viewport_width || m_height != viewport_height)
	{
		...
		if (m_p_swapchain)
		{
			...
			// Re-acquire waitable object after resize
			m_frame_latency_waitable = m_p_swapchain->GetFrameLatencyWaitableObject();
...
IDXGISwapChain4* RenderInterface_DX12::Get_SwapChain() const
{
	return m_p_swapchain;
}

ID3D12CommandQueue* RenderInterface_DX12::Get_CommandQueue() const
{
	return m_p_command_queue;
}
...
void RenderInterface_DX12::Initialize_Swapchain(int width, int height) noexcept
{
	...
	{
		RECT rc;
		GetClientRect(static_cast<HWND>(m_p_window_handle), &rc);
		if (width <= 0)
	...
	// Pace the CPU to match display refresh — prevents queuing too many frames
	m_p_swapchain->SetMaximumFrameLatency(1);
	m_frame_latency_waitable = m_p_swapchain->GetFrameLatencyWaitableObject();
